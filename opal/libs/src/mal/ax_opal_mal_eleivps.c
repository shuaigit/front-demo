...
static AX_S32 update_osd(AX_OPAL_MAL_ELE_HANDLE self, AX_OPAL_MAL_PROCESS_DATA_T *pPorcessData) {
    ...
    /* Step 3. update osd */
    AX_OPAL_MAL_IVPS_CHN_OSD_ATTR_T stOsd;
    memcpy(&stOsd, pOsd, sizeof(AX_OPAL_MAL_IVPS_CHN_OSD_ATTR_T));
    memcpy(&(stOsd.stOsdAttr), pData->pstAttr, sizeof(AX_OPAL_OSD_ATTR_T));
    nRet = UpdateRgn((AX_OPAL_MAL_ELE_HANDLE)pEle, nGrpId, nChnId, &stOsd);
    if (AX_SUCCESS == nRet) {
        memcpy(&(pOsd->stOsdAttr), pData->pstAttr, sizeof(AX_OPAL_OSD_ATTR_T));
    }
        ...
        case AX_OPAL_MAINCMD_VIDEO_SETSNSATTR:
            switch (pPorcessData->eSubCmdType) {
                case AX_OPAL_SUBCMD_SNSATTR_FPS:
                    nRet = set_fps(self, pPorcessData);
                    break;
