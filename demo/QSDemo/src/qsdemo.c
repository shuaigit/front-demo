...
#define CONDITION1_MASK_SKEL  0x1       //  0001
#define CONDITION2_MASK_VENC0 0x2       //  0010
#define CONDITION3_MASK_VENC1 0x4       //  0100
#define CONDITION4_MASK_STOP  0x8       //  1000
#define CONDITION4_MASK_AUDIO 0x10      // 10000

static unsigned int g_conditions_sleep = CONDITION0_MASK_NONE
                                        | CONDITION2_MASK_VENC0
                                        | CONDITION3_MASK_VENC1
                                        | CONDITION4_MASK_AUDIO;
...
static int get_required_mask() {
    ...
    sleep_flag |= CONDITION1_MASK_SKEL;
    sleep_flag |= CONDITION2_MASK_VENC0;

    if (g_entry_param.nCamCnt > 1) {
        sleep_flag |= CONDITION3_MASK_VENC1;
    }

#ifdef QSDEMO_AUDIO_SUPPORT
    sleep_flag |= CONDITION4_MASK_AUDIO;
#endif
...
static int check_sleep_condition() {
    ...
    while ((g_conditions_sleep & required_mask) != required_mask) {
        if ((g_conditions_sleep & CONDITION4_MASK_STOP) == CONDITION4_MASK_STOP) {
            ret = 1;
            break;
        }
...
static void update_sleep_condition(int mask, bool bSetTrue) {
    pthread_mutex_lock(&g_mutex_sleep);
    ...
    if ((g_conditions_sleep & required_mask) == required_mask
        || (g_conditions_sleep & CONDITION4_MASK_STOP) == CONDITION4_MASK_STOP) {
...
AX_VOID AudioPlayCallback(const SAMPLE_AUDIO_PLAY_FILE_RESULT_PTR pstResult) {
    ...
    update_sleep_condition(CONDITION4_MASK_AUDIO, true);
}
#endif
...
AX_S32 QSNotifyEventCallback(const AX_NOTIFY_EVENT_E event, AX_VOID * pdata) {
    if (event == AX_NOTIFY_EVENT_SLEEP) {
        ...
        update_sleep_condition(CONDITION2_MASK_VENC0, false);
        update_sleep_condition(CONDITION3_MASK_VENC1, false);
        update_sleep_condition(CONDITION1_MASK_SKEL, false);
...
AX_VOID PrintLaunchTimeResult()
{
    ...
    ReadISPTimes(&u64Started[0], &u64Opended[0], &u64FrameReady[0], &u64OutToNext[0], g_entry_param.nCamCnt);

    LogToFile("main launch timestamp:  %llu", g_u64LaunchPts);
    if (g_entry_param.nCamCnt == 1) {
        LogToFile("round[%d] latency result: 1st Raw Image: %llu, 1st Detect Result: %llu, 1st Venc Frame: %llu, main launch: %llu.",
                g_nRunningTimes,
                u64FrameReady[0],
        ...
        LogToFile("round[%d] latency result: 1st Raw Image: [%llu, %llu], 1st Detect Result: [%llu, %llu], 1st Venc Frame: [%llu, %llu] main launch: %llu.",
                g_nRunningTimes,
                u64FrameReady[0],
                u64FrameReady[1],
                g_stResultTimes.u64DetectTimeFromBoot[0],
    ...
    LogToFile("round[%d] vin latency sns[0]: started: %llu, opened: %llu, 1st raw ready: %llu, out next: %llu",
              g_nRunningTimes,
              u64Started[0],
              u64Opended[0],
              u64FrameReady[0],
    ...
    if (g_entry_param.nCamCnt == 2) {
        LogToFile("round[%d] vin latency sns[1]: started: %llu, opened: %llu, 1st raw ready: %llu, out next: %llu",
              g_nRunningTimes,
              u64Started[1],
              u64Opended[1],
              u64FrameReady[1],
    ...
    while (pstPara->bThreadStart && !ThreadLoopStateGet()) {
        ...
        if (AX_SUCCESS == s32Ret) {
            if (g_nQSType == QSAOV && g_nLastFps == 1 && (pstPara->nVeChn % IVPSChannelNumber) == 0) {
                ...
                AX_SYS_SleepTimeStamp(AX_ID_USER, E_STS_GET_VENC);
            }
            ...
            if (g_nQSType == QSAOV && (pstPara->nVeChn % IVPSChannelNumber) == 0) {
                if (stStream.stPack.pu8Addr && stStream.stPack.u32Len > 0) {
                    ...
                    if (bVencAllFrmGotFromWakeup) {
                        ...
                        if (pstPara->nSndId == 0) {
                            update_sleep_condition(CONDITION2_MASK_VENC0, true);
                        } else if (pstPara->nSndId == 1) {
                            update_sleep_condition(CONDITION3_MASK_VENC1, true);
                        }
...
static AX_S32 UpdateSnsFps(SAMPLE_ENTRY_PARAM_T *pEntryParam, AX_S32 nFps) {
    ...
    for (AX_S32 i = 0; i < pEntryParam->nCamCnt; i++) {
        ...
        if (gCams[i].bMipiSwitchEnable) {
            AX_S32 nCurSns = COMMON_CAM_GetCurSnsSwitch();
            if (nCurSns == 1) {
...
static AX_VOID NoBodyFoundNotify() {
    ...
    if (g_nQSType == QSReboot) {
        ...
        if (!g_bSleepFlag) {
            ...
            update_sleep_condition(CONDITION1_MASK_SKEL, true);
...
static AX_VOID SkelResultCallback(AX_SKEL_HANDLE pHandle, AX_SKEL_RESULT_T *pstResult, AX_VOID *private) {
    ...
    if (g_nQSType == QSAOV && g_nLastFps == 1) {
        ...
        AX_SYS_SleepTimeStamp(AX_ID_USER, E_STS_GET_SKEL);
    }
...
static AX_VOID *AudioFilePlayTriggerThread(AX_VOID* param) {
    ...
    while (!gLoopExit) {
        ...
        if (!gLoopExit
            ...
            if (s32Ret == 0) {
                update_sleep_condition(CONDITION4_MASK_AUDIO, false);
            }
        }
    }
...
static AX_VOID *GetFrameThreadForAI(AX_VOID *pThreadParam)
{
    ...
    while (!gLoopExit) {
        ...
        if (g_nQSType == QSAOV && g_nLastFps == 1) {
            // for debug: timestamp get skel result
            AX_SYS_SleepTimeStamp(AX_ID_USER, E_STS_GET_FRAME);
        }
...
static AX_S32 QSDemoRun(SAMPLE_ENTRY_PARAM_T *pEntryParam)
{
    ...
    gLoopExit = 1;
    update_sleep_condition(CONDITION4_MASK_STOP, true);
